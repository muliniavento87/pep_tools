...
exist=$(which pepapps);
if [ -n ${exist} ]; then
    echo "Rimozione peplinks ..."
    # rimozione .desktop
    v0="rm /usr/share/applications/applet-peplinks.desktop"
    v1="rm ${HOME}/.config/autostart/applet-peplinks.desktop"
    # rimozione link globale
    v2="rm /usr/local/bin/peplinks"
    # rimozione cartella
    v3="rm -r /usr/local/PepAppsLinks/"
    # Messaggio di avviso OK
    v4="echo 'OK peplinks rimosso'"
