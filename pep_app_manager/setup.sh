...
exist=$(which pepapps);
if [ -z ${exist} ]; then
    echo "Installo pepapps ..."
    # creo cartella
    v0="cp -r ${PATH_SETUP}/pepapps /usr/local/PepAppsManager/"
    # link globale
    v1="ln -s /usr/local/PepAppsManager/start.sh /usr/local/bin/pepapps"
    # link menu
    v2="cp ${PATH_SETUP}/pepapps/applet-pepapps.desktop /usr/share/applications/applet-pepapps.desktop"
    # autostart
    v3="cp ${PATH_SETUP}/pepapps/applet-pepapps.desktop ${HOME}/.config/autostart/applet-pepapps.desktop"
    # set permessi utente per file configurazioni
    v4="chmod 777 /usr/local/PepAppsManager/store.json"
    # Messaggio di avviso OK
    v5="echo 'OK pepapps installato'"
